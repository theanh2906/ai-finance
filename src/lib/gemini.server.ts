      ...
      items: {
        ...
        properties: {
          category: {
            ...
            description: 'Vietnamese translated categories like Sức khỏe & Y tế, Ăn uống & Siêu thị, ...',
          },
          ...
          note: {
            type: SchemaType.STRING,
            description: 'Summarized note about the largest expenses or related details in this category',
          },
        },
...
const statementSchema: Schema = {
  ...
  properties: {
    summary: {
      ...
      required: ['bankName', 'accountHolder', 'period', 'totalIncome', 'totalExpense', 'netBalance', 'currency'],
    },
    transactions: {
      type: SchemaType.ARRAY,
      items: {
        ...
        properties: {
          ...
          category: { type: SchemaType.STRING, description: 'e.g., Food, Rent, Salary, etc.' },
        },
...
const payslipSchema: Schema = {
  ...
  properties: {
    summary: {
      ...
      required: ['employerName', 'employeeName', 'payPeriod', 'grossPay', 'netPay', 'totalDeductions', 'currency'],
    },
    deductions: {
      type: SchemaType.ARRAY,
      items: {
        ...
        properties: {
          description: { type: SchemaType.STRING },
          amount: { type: SchemaType.NUMBER },
          type: { type: SchemaType.STRING, description: 'tax | insurance | pension | other' },
        },
...
export async function analyzeDocumentServer(
  ...
  if (!apiKey) throw new Error('API key is required. Please set your Gemini API Key.');

  const genAI = new GoogleGenerativeAI(apiKey);

  let schema = statementSchema;
  let prompt = 'Analyze this bank statement. Provide a summary and a list of transactions.';

  if (type === 'payslip') {
    schema = payslipSchema;
    prompt = 'Analyze this payslip. Provide a summary including Gross Pay, Net Pay, and a detailed list of deductions.';
  ...
  try {
    ...
    throw new Error(`Failed to analyze ${type}. ${rawError.message || 'Please check API key and file format.'}`);
  }
}
